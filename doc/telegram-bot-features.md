...
## 2. Feature Implementation Status in CoCo
...
| **InlineKeyboardMarkup** | ✅ | Used extensively: session list, history pagination, directory browser, interactive UI |
...
| 2 | **Expandable blockquote for thinking** | Medium | Low | Wrap Codex reasoning in `<blockquote expandable>` for cleaner layout. Replaces spoiler approach and keeps long thought blocks easier to scan |
...
### Currently Forwarded by CoCo

These core commands are registered in the Telegram bot menu and forwarded to Codex:

| Command | Bot Menu Description | Function |
|---------|---------------------|----------|
| `/clear` | ↗ Clear conversation history | Wipes conversation, starts fresh. CoCo also clears session association |
...
Consider adding `/context` to the forwarded command allowlist — it is parameterless, non-interactive, and provides useful context window usage info that complements `/cost`.
