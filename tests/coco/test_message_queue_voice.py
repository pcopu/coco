...
@pytest.mark.asyncio
async def test_process_content_task_honors_one_shot_response_mode_override(monkeypatch):
    text_sends: list[tuple[int, str, dict[str, object]]] = []
    voice_sends: list[tuple[int, str, bytes, dict[str, object]]] = []
    ...
    monkeypatch.setattr(
        mq.session_manager,
        "get_topic_response_mode",
        lambda _user_id, _thread_id, *, chat_id=None: "text",
    )
    ...
    async def _synthesize_voice_note(text: str):
        assert text == "One-shot spoken reply"
        return "audio/ogg", b"voice-bytes"
    ...
    task = mq.MessageTask(
        task_type="content",
        window_id="@1",
        parts=["One-shot spoken reply"],
        text="One-shot spoken reply",
        content_type="text",
        thread_id=77,
