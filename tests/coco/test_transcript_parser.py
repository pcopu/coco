"""Tests for coco.transcript_parser — pure logic, no I/O."""

import pytest

from coco.transcript_parser import (
    ParsedMessage,
    TranscriptParser,
)
...
class TestParseEntries:
    ...
    def test_codex_response_item_phase_maps_to_progress_and_final(self):
        entries = [
            {
                "type": "event_msg",
        ...
        result, pending = TranscriptParser.parse_entries(entries)
        assert not pending
        assert [e.content_type for e in result] == ["text", "thinking", "progress", "text"]
        assert result[2].text == "working update"
        assert result[3].text == "final answer"

    def test_codex_response_item_unknown_phase_stays_progress(self):
        entries = [
            {
                "type": "response_item",
                "timestamp": "2026-01-01T00:00:00Z",
        ...
        assert not pending
        assert len(result) == 1
        assert result[0].content_type == "progress"
        assert result[0].text == "still working"

    def test_codex_response_item_call_emits_native_progress_summary(self):
        entries = [
            {
                "type": "response_item",
                "timestamp": "2026-01-01T00:00:00Z",
                "payload": {
                    ...
                    "action": {
                        "type": "search",
                        "query": "site:support.google.com sender guidelines",
                    },
                },
            }
        ]
        ...
        assert result[0].role == "assistant"
        assert result[0].content_type == "progress"
        assert result[0].event_type == "response_item:web_search_call"
        assert result[0].text == "web search: site:support.google.com sender guidelines"

    def test_codex_event_agent_message_fallback_without_response_item(self):
        entries = [
            {
                "type": "event_msg",
                "timestamp": "2026-01-01T00:00:00Z",
                "payload": {"type": "agent_message", "message": "fallback text"},
            }
        ]
    ...
    def test_codex_task_complete_emits_lifecycle_entry(self):
        entries = [
            {
                "type": "event_msg",
        ...
        assert result[0].content_type == "lifecycle"
        assert result[0].event_type == "task_complete"
