...
def test_build_model_info_text_is_compact_and_uses_greeting(tmp_path, monkeypatch):
    ...
    assert "Future turns in this topic use this without resetting the bound session." in text
...
@pytest.mark.asyncio
async def test_model_callback_preserves_thread_on_model_change(monkeypatch):
    monkeypatch.setattr(SessionManager, "_load_state", lambda self: None)
    monkeypatch.setattr(SessionManager, "_save_state", lambda self: None)
    mgr = SessionManager()
    mgr.bind_topic_to_codex_thread(
...
@pytest.mark.asyncio
async def test_model_effort_callback_preserves_thread_on_change(monkeypatch):
    monkeypatch.setattr(SessionManager, "_load_state", lambda self: None)
    monkeypatch.setattr(SessionManager, "_save_state", lambda self: None)
    mgr = SessionManager()
    mgr.bind_topic_to_codex_thread(
    ...
    class _FakeQuery:
        def __init__(self) -> None:
            self.data = f"{CB_MODEL_EFFORT_SET}xhigh"
            self.message = type(
                "Msg",
                (),
                {
    ...
    assert edits
    assert "Topic reasoning: `xhigh`" in edits[-1]
