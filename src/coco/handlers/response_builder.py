...
def build_response_parts(
    ...
    """
    # Preserve delta whitespace for assistant progress/thinking streams.
    # Tokenized app-server deltas may carry leading spaces that are required
    # to reconstruct readable text.
    preserve_delta_spacing = role == "assistant" and content_type in {
        "thinking",
        "progress",
    }
    if preserve_delta_spacing:
        text = text
    else:
        text = text.strip()
